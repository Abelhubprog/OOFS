...
async fn helius_webhook(
    ...
    state.metrics.increment_counter("helius_webhooks_received_total");

    // Verify HMAC signature
    let signature = get_helius_sig_from_headers(&headers);
    if !verify_webhook_signature(&state.cfg.helius_webhook_secret, &body, signature.as_deref()) {
        warn!("Invalid webhook signature");
        state.metrics.increment_counter("helius_webhooks_invalid_signature_total");
    ...
    let notifications: Vec<HeliusTransactionNotification> = match serde_json::from_slice(&body) {
        ...
        Err(e) => {
            ...
            state.metrics.increment_counter("helius_webhooks_parse_errors_total");
            return StatusCode::BAD_REQUEST;
        }
    };

    info!("Processing {} transaction notifications", notifications.len());
    ...
    for notification in notifications {
        match process_transaction_notification(&state, &notification).await {
            ...
            Err(e) => {
                error!("Failed to process transaction {}: {}", notification.signature, e);
                error_count += 1;
            }
        }
    }
    ...
    state.metrics.observe_histogram("helius_webhook_processing_seconds", processing_time.as_secs_f64());
    state.metrics.increment_counter_by("helius_transactions_processed_total", processed_count);
    
    if error_count > 0 {
        state.metrics.increment_counter_by("helius_transactions_errors_total", error_count);
    }
...
async fn process_transaction_notification(
    ...
    let storage_key = format!("tx/{}/{}.json.zst", &notification.signature[0..2], notification.signature);
    ...
    if let Some(transfers) = &notification.token_transfers {
        for (idx, transfer) in transfers.iter().enumerate() {
            ...
            let amount = transfer.token_amount.as_ref()
    ...
    if let Some(native_transfers) = &notification.native_transfers {
        for (idx, transfer) in native_transfers.iter().enumerate() {
            ...
            let token_transfer_count = notification.token_transfers.as_ref().map(|t| t.len()).unwrap_or(0);
...
async fn refresh_token_price(state: &AppState, mint: &str) -> anyhow::Result<()> {
    // Implementation would call Jupiter API or other price sources
...
async fn metrics_cleanup_worker(state: AppState) {
    ...
    loop {
        ...
        let cleanup_result = sqlx::query!(
            "DELETE FROM tx_raw WHERE ts < NOW() - INTERVAL '30 days'"
        )
        .execute(&state.pg.0)
        .await;
...
async fn health_check(State(state): State<AppState>) -> Json<serde_json::Value> {
    ...
    let store_ok = state.store.put("health/indexer-check.txt", b"ok").await.is_ok();
...
async fn webhook_stats(State(state): State<AppState>) -> Json<WebhookStats> {
    ...
    let processed_count = sqlx::query_scalar!(
        "SELECT COUNT(*) FROM tx_raw WHERE ts >= NOW() - INTERVAL '24 hours'"
    )
    .fetch_one(&state.pg.0)
    .await
    .unwrap_or(0) as u64;
    
    let last_processed = sqlx::query_scalar!(
        "SELECT ts FROM tx_raw ORDER BY ts DESC LIMIT 1"
    )

...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/analyze-wallet', authenticateUser, async (req: any, res) => {
    try {
      ...
      // Connect to real backend for wallet analysis
  ...
  app.get('/api/campaigns', async (req, res) => {
    try {
      // Connect to real backend at http://localhost:5000
      const backendUrl = process.env.BACKEND_URL || 'http://localhost:5000';
      const response = await fetch(`${backendUrl}/v1/campaigns`);
  ...
  app.get('/api/campaigns/participations', async (req, res) => {
    try {
      // Connect to real backend for participations
      const backendUrl = process.env.BACKEND_URL || 'http://localhost:5000';
      const response = await fetch(`${backendUrl}/v1/campaigns/participations`);
      ...
      console.error("Error fetching participations from backend:", error);
